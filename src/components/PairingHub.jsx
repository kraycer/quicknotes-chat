...
import { Copy, Plus, LogIn, ShieldCheck, User, Sparkles, Lock, Check, MessageSquare, Trash2, ChevronRight, Mic, Camera, FileText, Search, Edit2 } from 'lucide-react';
...
export default function PairingHub({ userId, onJoinRoom, onLockApp }) {
  ...
  // Persistent list of active rooms
  ...
  const handleJoinWithCode = (e) => {
    ...
    if (!cleanCode || cleanCode.length < 5) {
      setError('Ingresa un código válido (ej: 849-102)');
      return;
    }
    setError('');
  ...
  return (
    <div className="flex flex-col h-full h-[100dvh] bg-[#0B0F17] text-gray-100 p-3 sm:p-4 max-w-md sm:max-w-lg mx-auto safe-pt safe-pb overflow-hidden">
      {/* Top Header with Notch Margin */}
      <header className="flex items-center justify-between py-3 mb-2 border-b border-gray-800/80 pt-4 sm:pt-6">
        <div className="flex items-center gap-2.5">
          <div className="w-10 h-10 bg-cyan-500/10 text-cyan-400 rounded-2xl border border-cyan-500/20 flex items-center justify-center">
        ...
        <button
          onClick={onLockApp}
          className="p-2.5 text-gray-400 hover:text-red-400 bg-[#151C28] rounded-2xl border border-gray-800 transition active:scale-95"
      ...
      <div className="flex-1 overflow-y-auto space-y-4 pr-1">
        {/* User Identity Card */}
        <div className="bg-[#151C28] border border-gray-800 rounded-3xl p-4 shadow-xl relative overflow-hidden">
          ...
          <div className="flex items-center justify-between bg-[#0B0F17] border border-gray-800/80 rounded-2xl p-3">
            <span className="text-base sm:text-lg font-mono font-bold tracking-wider text-gray-100">
              {userId}
            </span>
            <button
        ...
        {/* Room Action Buttons */}
        <div className="bg-[#151C28] border border-gray-800 rounded-3xl p-4 shadow-xl space-y-3">
          <button
          ...
          >
            <Sparkles className="w-4 h-4" /> Crear Nueva Sala (Código Aleatorio)
          </button>
          ...
          <form onSubmit={handleJoinWithCode} className="flex gap-2">
            ...
            <button
              type="submit"
              className="px-4 py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white rounded-2xl font-medium text-xs transition flex items-center gap-1 shrink-0 shadow-md"
            >
        ...
        <div className="space-y-2">
          ...
          {recentRooms.length > 2 && (
            <div className="relative mb-2">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-2 bg-[#151C28] border border-gray-800 rounded-xl text-xs text-gray-200 outline-none focus:border-cyan-500/40"
              />
            </div>
          )}
          ...
          {filteredRooms.length === 0 ? (
            <div className="p-6 text-center bg-[#151C28]/60 border border-gray-800/80 rounded-3xl text-gray-500 text-xs">
              No hay salas activas. Crea una sala o introduce un código para chatear.
            </div>
          ) : (
            <div className="space-y-2">
              {filteredRooms.map((code) => {
                ...
                return (
                  <div
                    ...
                    className="p-3.5 bg-[#151C28] border border-gray-800/90 rounded-2xl hover:border-cyan-500/50 transition cursor-pointer flex items-center justify-between group active:scale-98 shadow-md"
                  >
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      {/* Avatar Circle */}
                      <div className="w-12 h-12 rounded-full bg-gradient-to-br from-cyan-600/30 to-blue-600/30 border border-cyan-500/30 text-cyan-300 flex items-center justify-center font-mono font-bold text-sm shrink-0 shadow-inner">
                        {roomAlias ? roomAlias.slice(0, 2).toUpperCase() : code.slice(0, 3)}
                      </div>

                      {/* Info & Last Message */}
                      <div className="min-w-0 flex-1 pr-1">
                        <div className="flex items-center justify-between mb-1">
                          <h3 className="font-bold text-sm text-gray-100 group-hover:text-cyan-400 transition truncate">
                            {roomAlias || `Sala ${code}`}
                          </h3>
                        ...
                        <div className="flex items-center gap-1 text-xs text-gray-400 truncate">
                          {lastMsg ? (
                            ...
                            <span className="text-gray-500 italic text-[11px]">Conversación vacía</span>
                          )}
                        </div>
                      </div>
                    </div>
