...
import { Clock, CheckCheck, Check, Reply, Play, Pause, ChevronDown, Pin } from 'lucide-react';
    ...
    <div className="flex-1 flex flex-col min-h-0 relative overflow-hidden bg-[#0B0F17]">
        ...
        <div className="bg-[#151C28]/95 border-b border-cyan-500/30 px-4 py-2 flex items-center justify-between z-10 relative backdrop-blur-md shrink-0">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <span className="text-[10px] font-mono text-cyan-400 font-semibold block">Mensaje Fijado</span>
              <p className="text-xs text-gray-200 truncate">{pinnedMessage.content || '[Archivo multimedia]'}</p>
            </div>
          </div>
          <button onClick={() => onPinMessage(null)} className="text-[11px] text-gray-400 hover:text-gray-200 px-2 py-1">
            Quitar
          </button>
        </div>
      )}
              ...
              >
                ...
                >
                  <div
                    ...
                    className={`max-w-[84%] sm:max-w-[72%] rounded-2xl p-3 sm:p-3.5 shadow-lg relative ${
                      isMe
                  ...
                  >
                    {/* Reaction Popup Matrix */}
                    {activeReactionMsgId === msg.id && (
                      <div className={`absolute -top-10 ${isMe ? 'right-0' : 'left-0'} bg-[#151C28] border border-cyan-500/30 rounded-full px-2 py-1 shadow-2xl flex items-center gap-1 z-30 animate-fadeIn`}>
                        {REACTION_EMOJIS.map((emoji) => (
                          <button
                            ...
                            className="w-7 h-7 rounded-full hover:bg-gray-800 flex items-center justify-center text-sm transition active:scale-125"
                          >
                            {emoji}
                          </button>
                        ))}
                    ...
                    {msg.type === 'audio' && (
                      <div className="flex items-center gap-3 py-1">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            toggleAudio(msg.id, msg.media_url);
                          }}
                    ...
                    {msg.type === 'photo' && (
                      <div className="my-1 rounded-xl overflow-hidden border border-black/20">
                        <img src={msg.media_url} alt="Foto efímera" className="max-h-60 w-full object-cover" />
                      </div>
                    )}
                    ...
                    {msg.reactions && Object.keys(msg.reactions).length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        {Object.entries(msg.reactions).map(([emoji, count]) => (
                          <span key={emoji} className="text-xs bg-black/20 px-1.5 py-0.5 rounded-md">
                            {emoji} {count > 1 ? count : ''}
                          </span>
                        ))}
                      </div>
      ...
      {showScrollBottom && (
        <button
          ...
          className="absolute bottom-4 right-4 p-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded-full shadow-2xl transition active:scale-90 z-20 flex items-center justify-center border border-cyan-400/30"
